...
def speakerDiarization(filename, n_speakers, mt_size=2.0, mt_step=0.2, 
    ...
    # "fs" is the Frame Rate of the Audio
    # "x" is the array(of type "short") where each element is a frame and each frame/element contains
    # uncompressed samples for each channel 
    ...
    if plot_res:
        ...
        ax1.axis((0, duration, -1, len(class_names)))
#CHANGED CODE##############################################################################################
        ayush_var = numpy.array(range(len(cls)))*mt_step+mt_step/2.0
        plt.xticks(np.arange(0, max(ayush_var)+1, 1))
        #ax1.set_xticklabels(np.arange(min(ayush_var), max(ayush_var)+1, 0.5))
        plt.grid(which = 'major' , linestyle = '-', linewidth = '0.5', color = 'red')
        plt.grid(which = 'minor' , linestyle = ':', linewidth = '0.5', color = 'black')

        import sys

        print("x= ",numpy.array(range(len(cls)))*mt_step+mt_step/2.0)
        print("y= ",cls)
###########################################################################################################
        ax1.set_yticklabels(class_names)
        ax1.plot(numpy.array(range(len(cls)))*mt_step+mt_step/2.0, cls)

# Plotting of Ground Truth File
    if os.path.isfile(gt_file):
        if plot_res:
    ...
    f = open("plot_data","w+")
    f.write(numpy.array(range(len(cls)))*mt_step+mt_step/2.0)
    f.write(cls)
    f.close()
    return cls
